...
global {
	...
	action update_day {
		...
		if day < close_days[0] {
			...
			comm_open_pct <- 1-(close_pcts[0]*scale);
			work_open_pct <- 1-(close_pcts[0]*scale);
		} else if day < close_days[2] {
			comm_open_pct <- 1-(close_pcts[1]*scale);
			work_open_pct <- 1-(close_pcts[1]*scale);
		} else if day < close_days[3] {
			comm_open_pct <- 1-(close_pcts[2]*scale);
			work_open_pct <- 1-(close_pcts[2]*scale);
		} else {
			comm_open_pct <- 1-(close_pcts[3]*scale);
			work_open_pct <- 1-(close_pcts[3]*scale);
		}
	}
