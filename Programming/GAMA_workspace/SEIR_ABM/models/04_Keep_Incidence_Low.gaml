/***
* Name: 04KeepIncidenceLow
* Author: Michael L. Jackson
* Description: Run the model keeping incidence <10/100000 over two weeks 
***/

model KeepIncidenceLow

/* Fourth program in the sequence of evaluating the impact of different COVID-19 control measures. */
/* After running interventions as actually observed, look at the level of continued social 		   */
/* distancing needed to keep new cases at the level needed to move to Phase 2 in Washington State, */
/* which is <10 new cases per 100,000 population over a 14 day period. Assuming 50% of cases end   */
/* being lab-detected, this translates to an incidence of < 7.3 new cases per day. 				   */
/* Starting on simulation day 121, modify the social distancing value (close_pcts) to maintain     */
/* that level. 																					   */
...
global {
	...
	list<int> close_days   <- [34,    41,    45,   57,  121];	// Simulation days when interventions change	
	list<float> close_pcts <- [0.395, 0.625, 0.67, 0.9, 0.8];	// Percent reductions in contacts at different intervention periods
	...
	action update_day {
		...
		} else if day < close_days[4] {
			comm_open_pct <- 1-close_pcts[3];
			work_open_pct <- 1-close_pcts[3];
		} else {
			comm_open_pct <- 1-close_pcts[4];
			work_open_pct <- 1-close_pcts[4];
		}
	}
}
...
experiment KeepStatusQuo type: batch repeat: 1 until: (day >= max_days) parallel: true {
